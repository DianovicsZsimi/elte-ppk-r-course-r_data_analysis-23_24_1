...
rm(rare_cocktai3, rare_cocktail1, rare_cocktail3, rare_cocktail4, rare_cocktail5, rare_cocktail6, x3, x5, students, sql, birth, bruh, datadrame, a, flights, flights_dt, dataset, planes, mapping, label_info)
...
for (row_index in 1:nrow(cheat_sheet_done)) {
  # Iterate through each specific ingredient column
  for (col_name in specific_ingredients_columns) {
    # Check if the ingredient is present for the cocktail (in ingredients1-6)
    if (col_name %in% cheat_sheet_done[row_index, 2:7]) {
      # If present, mark 'x' for that ingredient in the respective column
      cheat_sheet_done[row_index, col_name] <- "x"
    } else {
      cheat_sheet_done[row_index, col_name] <- ""  # Leave it empty if not present
    }
  }
}
